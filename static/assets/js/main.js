...
function updateScrollState() {
  ...
  if (thin)
    thin.classList.toggle(
      "scrolled",
      scrolled || document.body.classList.contains("menu-open"),
    );
}
...
function renderFaq() {
  ...
  const data = [
    ...
    // },
    {
      ...
      body: "Финансовая свобода → свободная жизнь → новый трек. Можно остаться, запустить новые проекты, или уйти в одиночное плавание. Главное — дойти до финиша вместе.",
    },
  ];
  container.innerHTML = "";
  ...
  container.querySelectorAll(".question-button").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      container
        ...
        .forEach((el) => el.classList.remove("answer-container-active"));
      container
        .querySelectorAll(".toggle-icon")
        .forEach((el) => el.classList.remove("toggle-icon-active"));
      if (!open) {
        item.classList.add("open");
        item
          .querySelector(".answer-container")
          ?.classList.add("answer-container-active");
