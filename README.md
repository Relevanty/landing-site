...
## Структура
- `static/` — готовые страницы (`main.html`, `community.html`, `strategy.html`, `faq.html`, `apply.html`, `contact.html`), общие стили/скрипты (`assets/css/main.css`, `assets/js/main.js`), ассеты (`images/`, `fonts/`), `CNAME`.
- `.github/workflows/deploy.yml` — деплой на GitHub Pages, берёт содержимое `static/`.
- `.gitignore` — базовые игноры логов/IDE.

## Запуск локально
- Открыть `static/main.html` в браузере (работает по `file://`).
